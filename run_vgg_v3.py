...
def my_generator(g, imgid2label, single_classes=single_classes):
    for batch_x, batch_id in g:
        batch_size = batch_x.shape[0]
        batch_y0 = np.zeros((batch_size, single_classes))
        batch_y1 = np.zeros((batch_size, single_classes))
        batch_y2 = np.zeros((batch_size, single_classes))
        for i in range(batch_size):
            labels = imgid2label[batch_id[i]]
            if(labels == None):
                continue
            for index in labels:
                if(index // single_classes<1):
                    batch_y0[i][index] = 1
                elif(index // single_classes<2):
                    batch_y1[i][index % single_classes] = 1
                else:
                    batch_y2[i][index % single_classes] = 1
        yield (batch_x,
               [batch_y0,batch_y1,batch_y2])
...
imgid2label = pickle.loads(open(os.path.join(data_path, "imgid2label.pkl"),"rb").read())
image_list = pd.DataFrame([i for i,_ in imgid2label.items()],columns=["id"])
image_list["num_id"] = image_list["id"]
...
TEST = False
if TEST:
    img_dir = os.path.join(data_path,"test_images/")
else:
    img_dir = os.path.join(data_path,"images/")

train_generator = datagen.flow_from_dataframe(image_list,
                                              x_col="id",
                                              y_col="num_id",
                                              has_ext=False,
                                              directory=img_dir,
                                              ...
                                              batch_size=bs,
                                              class_mode="other")
valid_generator = datagen.flow_from_dataframe(image_list,
                                              x_col="id",
                                              y_col="num_id",
                                              has_ext=False,
                                              directory=img_dir,
                                              ...
                                              batch_size=bs,
                                              class_mode="other")
my_train_generator = my_generator(train_generator, imgid2label)
my_valid_generator = my_generator(valid_generator, imgid2label)
if TEST:
    STEP_SIZE_TRAIN = 1
    STEP_SIZE_VALID = 1
